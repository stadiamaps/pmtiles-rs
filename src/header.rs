...
mod test {
    ...
    #[test]
    fn read_header() {
        let mut test = File::open("fixtures/stamen_toner_z3.pmtiles").expect("Unable to open test file.");
        let mut header_bytes = [0; 127];
        test.read_exact(header_bytes.as_mut_slice()).expect("Unable to read header.");
