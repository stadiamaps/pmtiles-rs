...
use crate::{AsyncBackend, Error, Header};
...
impl AsyncBackend for HttpBackend {
    async fn read_bytes(&self, dst: &mut [u8], offset: usize) -> Result<(), Error> {
        ...
        let range_header = req.headers_mut().entry(RANGE).or_insert(HeaderValue::from_static(""));
        let end = offset + dst.len() - 1;
        *range_header = HeaderValue::from_str(format!("bytes={offset}-{end}").as_str()).map_err(|_| Error::ReadError)?;
...
mod http_tests {
    ...
    async fn basic_http_test() {
        let client = reqwest::Client::builder()
            .use_rustls_tls().build().expect("Unable to create HTTP client.");
        let backend = HttpBackend::new(client, TEST_URL).expect("Unable to build HTTP backend.");

        let tiles = AsyncPmTiles::try_from_source(backend).await.expect("Unable to init PMTiles archive");
    }
}